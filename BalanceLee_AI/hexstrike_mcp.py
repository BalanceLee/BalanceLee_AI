...
# ColoredFormatter moved to shared utility to avoid duplication
# Using simplified logging for MCP client

# Setup simplified logging for MCP client
logging.basicConfig(
    level=logging.INFO,
...
def setup_mcp_server(hexstrike_client: HexStrikeClient) -> FastMCP:
    ...
    @mcp.tool()
    def sqlmap_scan(url: str, data: str = "", additional_args: str = "", cookies: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        ...
        if cookies:
            logger.info(f"{HexStrikeColors.HIGHLIGHT_YELLOW}🍪 Using cookies{HexStrikeColors.RESET}")
...
""",
    ...
    def list_directory(path: str = "~") -> Dict[str, Any]:
        logger.info(f"📁 Listing system directory via HTTP: {path}")
        params = {"directory": path}
        return hexstrike_client.safe_get("api/system/list-directory", params)

    @mcp.tool()
    def read_file(path: str, max_bytes: int = 4096) -> Dict[str, Any]:
        logger.info(f"📄 Reading system file via HTTP: {path} (max_bytes={max_bytes})")
        params = {"path": path, "max_bytes": max_bytes}
        return hexstrike_client.safe_get("api/system/read-file", params)

    @mcp.tool()
    def list_processes(limit: int = 50) -> Dict[str, Any]:
        logger.info(f"🧠 Listing system processes via HTTP (limit={limit})")
        params = {"limit": limit}
        return hexstrike_client.safe_get("api/system/list-processes", params)
    ...
    def http_framework_test(url: str, method: str = "GET", data: dict = {},
        ...
        【✅ 使用场景】
        - 简单的 HTTP 请求（GET/POST）
        - 不需要 JavaScript 执行的场景
        - 后端 API 测试（不依赖浏览器环境）
        - 获取页面源码和响应头
        
        【❌ 不适合场景】
        - 需要浏览器 Cookie/Session 的请求
        - 前端 API 调用（应使用 execute_browser_js）
        - 如果 POST 请求返回 400/403/401，说明需要浏览器上下文，应改用 execute_browser_js
        - 需要执行 JavaScript 的场景
    ...
    @mcp.tool()
    def browser_visit_page(url: str, timeout: int = 10, follow_redirects: bool = True, cookies: Dict[str, Any] = None) -> Dict[str, Any]:
        """访问URL并解析页面结构（表单、输入框、链接），发现可注入点。
        
        ⚠️ 重要限制：
        - 此工具只能GET请求访问页面，**不能提交表单或执行登录操作**
        - 如果返回的final_url包含"login"，说明需要先使用smart_login_attempt工具登录
        - 不要连续多次调用此工具访问同一个登录页面（会陷入循环）
        ...
        ❌ 不适用场景：
        - 登录操作（请使用smart_login_attempt）
        - 提交表单数据（请使用smart_login_attempt或其他工具）
        - 需要POST请求的操作
        ...
        Args:
            ...
            cookies: 会话cookies（可选，用于已认证的访问）

        Returns:
            {
                "success": bool,
                "final_url": str,  # 最终URL（可能被重定向）
                "is_login_page": bool,  # 是否是登录页面
                "forms": [...],  # 表单列表
                "injectable_points": int,  # 可注入点数量
                "has_source_code_link": bool,  # 是否有源码查看链接
                ...
            }
        """
        data = {
        ...
        logger.info(f"{HexStrikeColors.NEON_BLUE}🌐 Browser visiting: {url}{HexStrikeColors.RESET}")
        if cookies:
            logger.info(f"{HexStrikeColors.HIGHLIGHT_YELLOW}🍪 Using cookies{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/tools/browser_visit", data)

        if result.get("success"):
            forms_count = len(result.get("forms", []))
            links_count = result.get("total_links", 0)
            injectable_points = result.get("injectable_points", 0)

            logger.info(f"{HexStrikeColors.SUCCESS}✅ Page analysis completed{HexStrikeColors.RESET}")
            logger.info(f"  📝 Forms: {forms_count}")
            logger.info(f"  🔗 Links: {links_count}")
            logger.info(f"  💉 Injectable Points: {injectable_points}")

            if injectable_points > 0:
                logger.info(f"{HexStrikeColors.HIGHLIGHT_GREEN} INJECTABLE POINTS FOUND {HexStrikeColors.RESET}")
        else:
            logger.error(f"{HexStrikeColors.ERROR}❌ Browser visit failed for {url}{HexStrikeColors.RESET}")

        return result

    @mcp.tool()
    def execute_browser_js(url: str, js_code: str, wait_for_load: int = 3, cookies: Dict[str, Any] = None) -> Dict[str, Any]:
        """【⚠️ 优先使用场景 - 重要！】
        当遇到以下情况时，应该优先使用此工具：
        1. 🔴 发现前端 API 端点（如 /api/xxx, /api/monitor, /api/action）
        2. 🔴 HTTP POST 请求返回 400/403/401 错误（说明需要浏览器上下文）
        3. 🔴 需要浏览器 Cookie/Session/CSRF Token
        4. 🔴 CTF 题目要求在 Console 执行代码
        5. 🔴 需要调用 fetch/XMLHttpRequest 发送请求
        
        在浏览器 Console 执行 JavaScript 代码（使用 Playwright）
        
        🎯 核心功能：
        - 在真实浏览器环境中执行 JavaScript 代码
        - 支持异步操作（fetch、Promise 等）
        - 自动处理页面加载和 Cookie
        
        ✅ 典型使用场景：
        - 调用前端 API（如 fetch('/api/action', {...})）
        - 操作浏览器对象（document、window、localStorage）
        - 执行客户端 JavaScript 逻辑
        - 需要浏览器会话状态的操作
        
        ❌ 不适用场景：
        - 简单的 HTTP 请求（请使用 http_framework_test）
        - 不需要浏览器环境的操作
        ...
        1. 调用前端 API：
           execute_browser_js(
               url="http://target/",
               js_code="return fetch('/api/action', {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({action: 'secret'})}).then(r => r.json())"
           )
        ...
        3. 操作 DOM：
           execute_browser_js(
               url="http://target/",
               js_code="document.getElementById('button').click(); return document.body.innerText"
           )
        
        4. 读取 localStorage：
           execute_browser_js(
               url="http://target/",
               js_code="return localStorage.getItem('token')"
           )

        Args:
            url: 目标网址
            js_code: 要执行的 JavaScript 代码（支持 return 语句返回结果）
            wait_for_load: 等待页面加载的时间（秒，默认 3 秒）
            cookies: 会话 cookies（可选）

        Returns:
            {
                "success": bool,
                "result": Any,  # JavaScript 执行结果
                "result_type": str,  # 结果类型（string/number/object/array/null）
                "error": str  # 如果失败
            }
        """
        data = {
        ...
        logger.info(f"{HexStrikeColors.NEON_BLUE}🌐 Executing browser JS on: {url}{HexStrikeColors.RESET}")
        logger.info(f"{HexStrikeColors.HIGHLIGHT_CYAN}📜 JS Code: {js_code[:100]}...{HexStrikeColors.RESET}")
        
        result = hexstrike_client.safe_post("api/tools/execute_browser_js", data)

        if result.get("success"):
            result_preview = str(result.get("result", ""))[:200]
            logger.info(f"{HexStrikeColors.SUCCESS}✅ JavaScript executed successfully{HexStrikeColors.RESET}")
            logger.info(f"  📊 Result: {result_preview}")
            logger.info(f"  🔖 Type: {result.get('result_type', 'unknown')}")
        else:
            logger.error(f"{HexStrikeColors.ERROR}❌ JavaScript execution failed: {result.get('error', 'Unknown error')}{HexStrikeColors.RESET}")

        return result

    @mcp.tool()
    def browser_get_rendered_content(url: str, wait_time: int = 3, cookies: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        ✅ 适用场景：
        - 页面内容由 JavaScript 动态生成（React/Vue/Angular）
        - 需要看到 AJAX 加载的数据
        - 需要看到 JavaScript 修改后的 DOM
        - 初始 HTML 中没有关键信息
        
        ❌ 不适用场景：
        - 静态页面（请使用 browser_visit_page）
        - 不需要 JavaScript 渲染的页面
        
        💡 与 browser_visit_page 的区别：
        - browser_visit_page: 只获取初始 HTML（快速，但可能缺少动态内容）
        - browser_get_rendered_content: 获取 JS 执行后的完整内容（慢，但完整）

        Args:
            url: 目标网址
            wait_time: 等待页面渲染的时间（秒，默认 3 秒）
            cookies: 会话 cookies（可选）

        Returns:
            {
                "success": bool,
                "html": str,  # 完整渲染后的 HTML
                "text": str,  # 页面可见文本
                "title": str,  # 页面标题
        ...
        logger.info(f"{HexStrikeColors.NEON_BLUE}🌐 Getting rendered content from: {url}{HexStrikeColors.RESET}")
        
        result = hexstrike_client.safe_post("api/tools/browser_get_rendered_content", data)

        if result.get("success"):
            content_preview = result.get("text", "")[:200]
            logger.info(f"{HexStrikeColors.SUCCESS}✅ Rendered content retrieved{HexStrikeColors.RESET}")
            logger.info(f"  📄 Title: {result.get('title', 'N/A')}")
            logger.info(f"  📊 Content preview: {content_preview}...")
        else:
            logger.error(f"{HexStrikeColors.ERROR}❌ Failed to get rendered content: {result.get('error', 'Unknown error')}{HexStrikeColors.RESET}")

        return result

    @mcp.tool()
    def ask_user(question: str, options: list = None, context: str = "", session_id: str = "") -> Dict[str, Any]:
        """询问用户意见或获取用户指导（人机协作工具）
        
        【⭐ 重要】这是一个特殊的工具，用于与用户交互。
        
        🎯 使用场景（你自己决定何时使用）：
        1. 当你不确定下一步该怎么做时
        2. 当你发现重要信息（漏洞、flag等），想询问用户是否满意时
        3. 当你完成了一个阶段，想汇报进展并询问是否继续时
        4. 当你遇到困难或连续失败时，寻求用户的指导
        5. 当你需要用户的专业判断或决策时
        ...
        Args:
            ...
            options: 可选的选项列表（如 ["继续", "暂停", "修改方案"]）
            context: 背景信息（帮助用户理解当前状态）

        Returns:
            {
                "success": bool,
                "user_response": str,  # 用户的文本回复
                "user_choice": str,    # 用户选择的选项
                "timeout": bool        # 是否超时（超时会自动选择"继续"）
            }
        """
        data = {
            "question": question,
            "options": options or ["继续", "暂停", "提供建议"],
            "context": context,
            "session_id": session_id or "default"
        }
        
        logger.info(f"{HexStrikeColors.ELECTRIC_PURPLE}🤔 询问用户: {question}{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/tools/ask-user", data)
        
        if result.get("success"):
            user_response = result.get("user_response", "")
            logger.info(f"{HexStrikeColors.SUCCESS}✅ 用户回复: {user_response}{HexStrikeColors.RESET}")
        else:
            logger.error(f"{HexStrikeColors.ERROR}❌ 询问用户失败: {result.get('error', 'Unknown error')}{HexStrikeColors.RESET}")
    ...
    @mcp.tool()
    def crawl_site_endpoints(url: str, max_depth: int = 2, max_urls: int = 100,
                            include_subdomains: bool = False, additional_args: str = "", cookies: Dict[str, Any] = None) -> Dict[str, Any]:
        """爬取站点，发现所有可测试的端点（优先返回带参数的URL）。
        
        🕷️ 核心功能：
        - 递归爬取站点，发现所有可访问的URL
        - 优先返回带参数的URL（如?id=1&name=test）
        - 支持使用katana（如果可用）或自定义爬虫
        
        ✅ 适用场景：
        - 当browser_visit_page发现页面无可注入点时
        - 需要发现更多测试端点时
        - 寻找隐藏的管理页面或API端点
        
        💡 使用建议：
        - 如果目标URL没有参数，先用此工具爬取站点
        - 爬取结果中的urls_with_params可以直接用于漏洞测试
        - 配合discover_injectable_params使用效果更好

        Args:
            url: 目标URL（起始点）
            max_depth: 最大爬取深度（默认2，避免爬取过深）
            max_urls: 最大URL数量（默认100）
            include_subdomains: 是否包含子域名
            additional_args: 额外参数（用于katana）
            cookies: 会话cookies（可选，用于已认证的爬取）

        Returns:
            {
                "success": bool,
                "total_found": int,  # 总共发现的URL数量
                "params_found": int,  # 带参数的URL数量
                "urls_with_params": [...],  # 带参数的URL列表（优先）
                "all_urls": [...],  # 所有URL列表
                "tool": str  # 使用的工具（katana或custom）
            }
        """
        data = {
            "url": url,
        ...
        logger.info(f"{HexStrikeColors.ELECTRIC_PURPLE}🕷️  Starting site crawl: {url} (depth={max_depth}){HexStrikeColors.RESET}")
        if cookies:
            logger.info(f"{HexStrikeColors.HIGHLIGHT_YELLOW}🍪 Using cookies{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/tools/crawl_site", data)
        ...
        if result.get("success"):
            ...
            logger.info(f"{HexStrikeColors.SUCCESS}✅ Crawl completed using {tool_used}{HexStrikeColors.RESET}")
            logger.info(f"  🔍 Total URLs: {total_found}")
            logger.info(f"  💉 URLs with params: {params_found}")

            if params_found > 0:
                logger.info(f"{HexStrikeColors.HIGHLIGHT_GREEN} TESTABLE URLS FOUND {HexStrikeColors.RESET}")
        else:
            logger.error(f"{HexStrikeColors.ERROR}❌ Site crawl failed for {url}{HexStrikeColors.RESET}")

        return result

    @mcp.tool()
    def discover_injectable_params(url: str, method: str = "GET", test_depth: str = "basic", cookies: Dict[str, Any] = None) -> Dict[str, Any]:
        """深度分析URL，发现所有可注入参数（GET/POST/Cookie/Header）。
        ...
        - 分析Cookie参数
        - 分析可能的Header注入点
        - 生成测试用的URL（带测试payload）
        
        ✅ 适用场景：
        - 当browser_visit_page或crawl_site_endpoints发现URL后
        - 需要详细分析每个参数的注入可能性
        - 准备进行漏洞测试前的参数发现
        
        💡 使用建议：
        - 对于带参数的URL，使用此工具可以获得详细的注入点信息
        - 返回的injectable_points可以直接用于sqlmap、nuclei等工具
        - test_depth设置为"deep"可以发现隐藏参数

        Args:
            url: 目标URL
            method: HTTP方法（GET/POST）
            test_depth: 测试深度（basic/medium/deep）
            cookies: 会话cookies（可选）

        Returns:
            {
                "success": bool,
                "total_injectable_points": int,  # 总可注入点数量
                "discovered_params": {
                    "get_params": [...],  # GET参数列表
                    "post_params": [...],  # POST参数列表
                    "cookies": [...],  # Cookie参数列表
                    "headers": [...],  # Header参数列表
                    "injectable_points": [...]  # 详细的注入点信息
                },
                "recommendation": str  # 推荐的下一步操作
            }
        """
        data = {
            "url": url,
        ...
        logger.info(f"{HexStrikeColors.CYBER_ORANGE}🔍 Discovering parameters: {url} (depth={test_depth}){HexStrikeColors.RESET}")
        if cookies:
            logger.info(f"{HexStrikeColors.HIGHLIGHT_YELLOW}🍪 Using cookies{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/tools/discover_params", data)

        if result.get("success"):
            total_injectable = result.get("total_injectable_points", 0)
            discovered = result.get("discovered_params", {})
            get_params = len(discovered.get("get_params", []))
            post_params = len(discovered.get("post_params", []))
            cookies = len(discovered.get("cookies", []))
            headers = len(discovered.get("headers", []))

            logger.info(f"{HexStrikeColors.SUCCESS}✅ Parameter discovery completed{HexStrikeColors.RESET}")
            logger.info(f"  💉 Total Injectable Points: {total_injectable}")
            logger.info(f"  📥 GET params: {get_params}")
            logger.info(f"  📤 POST params: {post_params}")
            logger.info(f"  🍪 Cookies: {cookies}")
            logger.info(f"  📋 Headers: {headers}")

            if total_injectable > 0:
                logger.info(f"{HexStrikeColors.HIGHLIGHT_GREEN} READY FOR VULNERABILITY TESTING {HexStrikeColors.RESET}")
                recommendation = result.get("recommendation", "")
                if recommendation:
                    logger.info(f"  💡 {recommendation}")
        else:
            logger.error(f"{HexStrikeColors.ERROR}❌ Parameter discovery failed for {url}{HexStrikeColors.RESET}")

        return result

    @mcp.tool()
    def smart_login_attempt(url: str, username: str = "", password: str = "", try_defaults: bool = True) -> Dict[str, Any]:
        """智能登录工具 - 自动处理CSRF token和表单提交，完成登录操作。
        
        🔑 核心功能：
        - 自动提取登录表单和CSRF token（如user_token）
        - 自动尝试常见默认凭据（admin/password, admin/admin等）
        - 或使用用户提供的自定义凭据
        - 返回登录后的cookies，供后续请求使用
        
        ✅ 使用时机：
        - 当browser_visit_page返回is_login_page=True时
        - 当访问受保护资源被重定向到登录页时
        - 当final_url包含"login"时
        
        💡 工作流程：
        1. 访问登录页面，提取表单和CSRF token
        2. 尝试默认凭据或使用提供的凭据
        3. 提交POST请求完成登录
        4. 返回登录后的cookies（PHPSESSID等）
        
        ⚠️ 注意事项：
        - 如果默认凭据失败，会返回need_credentials=True，需要用户提供凭据
        - 登录成功后，cookies会自动保存到session中，后续请求会自动使用

        Args:
            url: 登录页面URL（如http://example.com/login.php）
            username: 用户名（可选，如果提供则优先使用）
            password: 密码（可选，如果提供则优先使用）
            try_defaults: 是否尝试默认凭据（默认True）
        ...
        Returns:
            {
                ...
                "cookies": {...},  # 登录后的cookies
                "username": str,  # 使用的用户名
                "final_url": str,  # 登录后的URL
                "need_credentials": bool,  # 是否需要用户提供凭据
                "tried_credentials": [...]  # 尝试过的凭据列表
            }
        """
        data = {
            "url": url,
        ...
        logger.info(f"{HexStrikeColors.FIRE_RED}🔐 Attempting smart login: {url}{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/tools/smart_login", data)

        if result.get("success"):
            logged_user = result.get("username", "unknown")
            logger.info(f"{HexStrikeColors.SUCCESS}✅ Login successful as: {logged_user}{HexStrikeColors.RESET}")
            
            cookies_count = len(result.get("cookies", {}))
            logger.info(f"  🍪 Cookies saved: {cookies_count}")
            logger.info(f"  🌐 Final URL: {result.get('final_url', url)}")
        elif result.get("need_credentials"):
            logger.warning(f"{HexStrikeColors.WARNING}⚠️  Default credentials failed, user input required{HexStrikeColors.RESET}")
            tried = result.get("tried_credentials", [])
            if tried:
                logger.info(f"  🔑 Tried: {', '.join(tried)}")
        else:
            logger.error(f"{HexStrikeColors.ERROR}❌ Login failed for {url}{HexStrikeColors.RESET}")
    ...
    @mcp.tool()
    def view_source_code(url: str, cookies: Dict[str, Any] = None) -> Dict[str, Any]:
        """查看后端源码（如DVWA的View Source按钮）。
        
        📄 核心功能：
        - 访问源码查看页面，获取后端代码
        - 自动识别编程语言（PHP、JavaScript、Python等）
        - 初步分析代码中的危险函数和用户输入处理
        
        ✅ 适用场景：
        - 当browser_visit_page发现has_source_code_link=True时
        - 需要白盒测试，分析代码逻辑时
        - 寻找硬编码的凭据或配置信息
        
        💡 使用建议：
        - 获取源码后，使用analyze_source_code工具进行深度分析
        - 源码分析可以发现逻辑漏洞和绕过方法
        - 配合intelligent_quick_test使用，可以生成针对性payload

        Args:
            url: 源码页面URL（如http://example.com/view_source.php）
            cookies: 会话cookies（必需，通常需要登录后才能查看源码）

        Returns:
            {
                "success": bool,
                "source_code": str,  # 源代码内容
                "source_code_length": int,  # 代码长度
                "language": str,  # 编程语言
                "analysis": {
                    "has_sql_query": bool,  # 是否包含SQL查询
                    "has_user_input": bool,  # 是否使用用户输入
                    "has_sanitization": bool,  # 是否有过滤/转义
        ...
        logger.info(f"{HexStrikeColors.CYBER_ORANGE}📄 Viewing source code: {url}{HexStrikeColors.RESET}")
        if cookies:
            logger.info(f"{HexStrikeColors.HIGHLIGHT_YELLOW}🍪 Using cookies{HexStrikeColors.RESET}")
        
        result = hexstrike_client.safe_post("api/tools/view_source", data)

        if result.get("success"):
            code_length = result.get("source_code_length", 0)
            language = result.get("language", "unknown")
            analysis = result.get("analysis", {})
            
            logger.info(f"{HexStrikeColors.SUCCESS}✅ Source code retrieved: {code_length} bytes ({language}){HexStrikeColors.RESET}")
            
            # 显示初步分析结果
            if analysis.get("has_sql_query"):
                logger.info(f"  🗄️  Contains SQL queries")
            if analysis.get("has_user_input"):
                logger.warning(f"  ⚠️  Uses user input")
            if analysis.get("has_sanitization"):
                logger.info(f"  🛡️  Has sanitization")
            else:
                logger.warning(f"  ⚠️  No sanitization detected")
            
            dangerous_funcs = analysis.get("dangerous_functions", [])
            if dangerous_funcs:
                logger.warning(f"{HexStrikeColors.HIGHLIGHT_RED} DANGEROUS FUNCTIONS: {', '.join(dangerous_funcs)} {HexStrikeColors.RESET}")
        else:
            logger.error(f"{HexStrikeColors.ERROR}❌ Failed to retrieve source code{HexStrikeColors.RESET}")

        return result

    @mcp.tool()
    def analyze_source_code(source_code: str, language: str = "php", url: str = "") -> Dict[str, Any]:
        """分析源码，识别潜在漏洞（SQL注入、XSS、命令注入等）。
        
        🔍 核心功能：
        - 静态代码分析，识别常见漏洞模式
        - 检测危险函数使用（eval、exec、system等）
        - 分析用户输入处理和过滤逻辑
        ...
        💡 使用建议：
        - 分析结果中的vulnerabilities包含具体的漏洞位置和类型
        - 根据分析结果，使用intelligent_quick_test快速验证
        - 或使用专业工具（sqlmap、nuclei）深度测试

        Args:
            source_code: 源代码内容（从view_source_code获取）
            language: 编程语言（php, javascript, python等）
            url: 源码对应的URL（可选，用于关联）

        Returns:
            {
                "success": bool,
                "vulnerability_count": int,  # 发现的漏洞数量
                "risk_level": str,  # 风险等级（CRITICAL/HIGH/MEDIUM/LOW）
                "vulnerabilities": [
                    {
                        "type": str,  # 漏洞类型（SQL Injection、XSS等）
                        "severity": str,  # 严重程度
        ...
        logger.info(f"{HexStrikeColors.BLOOD_RED}🔍 Analyzing {language} code ({len(source_code)} bytes){HexStrikeColors.RESET}")
        
        result = hexstrike_client.safe_post("api/tools/analyze_code", data)
        ...
        if result.get("success"):
            ...
            # 根据风险等级选择颜色
            risk_colors = {
                "CRITICAL": HexStrikeColors.VULN_CRITICAL,
                "HIGH": HexStrikeColors.VULN_HIGH,
                "MEDIUM": HexStrikeColors.VULN_MEDIUM,
                "LOW": HexStrikeColors.VULN_LOW
            }
            risk_color = risk_colors.get(risk_level, HexStrikeColors.INFO)
            
            logger.info(f"{HexStrikeColors.SUCCESS}✅ Code analysis complete{HexStrikeColors.RESET}")
            logger.info(f"  🚨 Vulnerabilities found: {vuln_count}")
            logger.info(f"  {risk_color}📊 Risk Level: {risk_level}{HexStrikeColors.RESET}")
            
            if summary:
                if summary.get("critical", 0) > 0:
                    logger.warning(f"  {HexStrikeColors.VULN_CRITICAL} CRITICAL: {summary['critical']} {HexStrikeColors.RESET}")
                if summary.get("high", 0) > 0:
                    logger.warning(f"  {HexStrikeColors.VULN_HIGH} HIGH: {summary['high']} {HexStrikeColors.RESET}")
                if summary.get("medium", 0) > 0:
                    logger.info(f"  {HexStrikeColors.VULN_MEDIUM} MEDIUM: {summary['medium']} {HexStrikeColors.RESET}")
            ...
            logger.error(f"{HexStrikeColors.ERROR}❌ Code analysis failed{HexStrikeColors.RESET}")
    ...
    @mcp.tool()
    def intelligent_quick_test(target: str, vuln_type: str = "xss", context: Dict[str, Any] = None,
                              cookies: Dict[str, str] = None, max_payloads: int = 5) -> Dict[str, Any]:
        """LLM驱动的智能快速测试 - 生成针对性Payload并快速验证漏洞。
        
        ⚡ 核心优势：
        - 速度快：3-10秒完成，比专业工具快10倍以上
        - 智能化：根据源码、技术栈、WAF情况生成针对性payload
        - 分层测试：快速测试 → 专业工具（如果需要）
        
        🎯 工作原理：
        1. 分析上下文（源码、表单、过滤规则）
        2. LLM生成针对性payload（而不是盲目fuzz）
        3. 快速测试5-10个高质量payload
        4. 根据结果决定是否需要深度扫描
        
        ✅ 适用场景（推荐使用）：
        - 有源码分析结果（从analyze_source_code获取）
        - 简单过滤或无WAF的情况
        - 需要快速验证漏洞存在性
        - 基础漏洞类型（XSS、SQLi、RCE、LFI）
        
        ❌ 不适用场景（建议用专业工具）：
        - 复杂WAF保护
        - 需要深度利用（如数据提取、getshell）
        - 复杂漏洞类型（XXE、反序列化等）
        
        💡 使用建议：
        - 先用此工具快速测试，如果发现漏洞且need_deep_scan=True，再用专业工具
        - context参数很重要，包含源码和表单信息可以提高成功率
        - 如果未发现漏洞但need_deep_scan=True，说明可能有复杂过滤，需要专业工具

        Args:
            target: 目标URL（必需）
            vuln_type: 漏洞类型（xss, sqli, rce, lfi, open_redirect）
            context: 上下文信息（可选但推荐，从browser_visit和analyze_code获取）
                {
                    "source_code": str,  # 源代码
                    "input_fields": [...],  # 表单字段
                    "url_params": {...},  # URL参数
                    "technology": str,  # 技术栈（php、nodejs等）
                    "has_waf": bool,  # 是否有WAF
                    "filters": [...]  # 已知的过滤规则
                }
            cookies: 会话Cookies（可选，用于已认证的测试）
            max_payloads: 最多测试几个Payload（默认5个，范围1-10）
        ...
        Returns:
            {
                ...
                "successful_payload": str,  # 成功的payload
                "payloads_tested": int,  # 测试的payload数量
                "need_deep_scan": bool,  # 是否需要深度扫描
                "recommended_tool": str,  # 推荐的专业工具（sqlmap、dalfox等）
        ...
        icon = vuln_icons.get(vuln_type, "🧪")
        
        logger.info(f"{HexStrikeColors.FIRE_RED}{icon} Starting intelligent quick test: {vuln_type.upper()} on {target}{HexStrikeColors.RESET}")
        
        result = hexstrike_client.safe_post("api/tools/intelligent_quick_test", data)
        ...
        if result.get("success"):
            ...
            need_deep_scan = result.get("need_deep_scan", False)
            reasoning = result.get("reasoning", "")
            recommended_tool = result.get("recommended_tool", "")
            
            if vulnerable:
                logger.info(f"{HexStrikeColors.HIGHLIGHT_RED} VULNERABILITY FOUND! {HexStrikeColors.RESET}")
                logger.info(f"  {HexStrikeColors.SUCCESS}✅ Type: {vuln_type.upper()}{HexStrikeColors.RESET}")
                logger.info(f"  {HexStrikeColors.CYBER_ORANGE}🎯 Confidence: {confidence:.0%}{HexStrikeColors.RESET}")
                logger.info(f"  {HexStrikeColors.BRIGHT_WHITE}💊 Payload: {successful_payload}{HexStrikeColors.RESET}")
                
                if need_deep_scan:
                    logger.info(f"  {HexStrikeColors.HIGHLIGHT_YELLOW} Recommend deep scan with {recommended_tool} {HexStrikeColors.RESET}")
            else:
                logger.info(f"{HexStrikeColors.INFO}ℹ️  No obvious vulnerability found{HexStrikeColors.RESET}")
                logger.info(f"  📊 Tested {payloads_tested} payloads")
                
                if need_deep_scan:
                    logger.info(f"  {HexStrikeColors.HIGHLIGHT_BLUE} Recommend deep scan with {recommended_tool} {HexStrikeColors.RESET}")
            
            logger.info(f"  💭 Reasoning: {reasoning}")
        else:
            logger.error(f"{HexStrikeColors.ERROR}❌ Quick test failed{HexStrikeColors.RESET}")

        return result

    # ============================================================================
    # ADVISORY SYSTEM TOOL - LLM驱动的知识顾问系统
    # ============================================================================

    @mcp.tool()
    def get_pentest_advisory(target_url: str, current_phase: str = "discovery", 
                           technology_stack: str = "", cms_detected: str = "",
                           waf_detected: str = "", has_login_page: bool = False,
                           has_file_upload: bool = False, has_search_function: bool = False,
                           is_authenticated: bool = False, executed_tools: str = "",
                           html_content: str = "", http_headers: str = "") -> Dict[str, Any]:
        """
        Get LLM-driven penetration testing advisory (suggestions only, not commands).
        
        This tool provides intelligent suggestions to help the LLM agent make autonomous decisions.
        The LLM agent has full authority to ignore any suggestions and make its own choices.
        
        Args:
            target_url: The target URL to analyze
            current_phase: Current penetration testing phase (discovery, parameter_analysis, vulnerability_scan, exploitation)
            technology_stack: Detected technologies (comma-separated)
            cms_detected: Detected CMS (WordPress, Drupal, etc.)
            waf_detected: Detected WAF/security system
            has_login_page: Whether a login page was found
            has_file_upload: Whether file upload functionality was found
            has_search_function: Whether search functionality was found
            is_authenticated: Whether currently authenticated
            executed_tools: Previously executed tools (comma-separated)
            html_content: HTML content for analysis (optional)
            http_headers: HTTP headers for analysis (optional, JSON string)
            
        Returns:
            Advisory information including tool suggestions, warnings, and knowledge
        """
        logger.info(f"{HexStrikeColors.ELECTRIC_PURPLE}🧠 Requesting penetration testing advisory for: {target_url}{HexStrikeColors.RESET}")
        ...
        data = {
            ...
            "executed_tools": [tool.strip() for tool in executed_tools.split(",") if tool.strip()],
            "context": {
                "current_phase": current_phase,
                "technology_stack": [tech.strip() for tech in technology_stack.split(",") if tech.strip()],
                "cms_detected": cms_detected if cms_detected else None,
                "waf_detected": waf_detected if waf_detected else None,
        ...
        result = hexstrike_client.safe_post("api/advisory/provide", data)
        ...
        if result.get("success"):
            ...
            # 统计建议数量
            total_suggestions = sum(len(advisories) for advisories in recommendations.values())
            high_confidence = len(recommendations.get("high_confidence", []))
            medium_confidence = len(recommendations.get("medium_confidence", []))
            
            logger.info(f"{HexStrikeColors.SUCCESS}✅ Advisory generated successfully{HexStrikeColors.RESET}")
            logger.info(f"  📊 Total suggestions: {total_suggestions}")
            logger.info(f"  🎯 High confidence: {high_confidence}, Medium confidence: {medium_confidence}")
            
            # 显示警告信息
            warnings = advisory.get("warnings", [])
            if warnings:
                logger.warning(f"{HexStrikeColors.WARNING}⚠️  Warnings: {len(warnings)}{HexStrikeColors.RESET}")
            ...
            if alerts:
                logger.info(f"{HexStrikeColors.INFO}ℹ️  Alerts: {len(alerts)}{HexStrikeColors.RESET}")
                for alert in alerts[:3]:  # 显示前3个提示
                    logger.info(f"    • {alert}")
            
            logger.info(f"{HexStrikeColors.HIGHLIGHT_PURPLE} Remember: These are suggestions only. LLM agent makes the final decisions. {HexStrikeColors.RESET}")
            
        else:
            logger.error(f"{HexStrikeColors.ERROR}❌ Advisory request failed{HexStrikeColors.RESET}")
            if "Advisory system not available" in result.get("error", ""):
                logger.error(f"  💡 Tip: Ensure GraphRAG advisory system is properly installed")
        
        return result

    @mcp.tool()
    def list_web_skills() -> Dict[str, Any]:
        """List available web pentest skills (LangGraph workflows)."""
        return hexstrike_client.safe_get("api/skills/list")

    @mcp.tool()
    def run_web_skill(skill_id: str, target_url: str, session_id: str = "", trace_id: str = "", cookies: Dict[str, Any] = None) -> Dict[str, Any]:
        """Run a web pentest skill by id with unified response envelope."""
