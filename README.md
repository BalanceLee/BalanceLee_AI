...
BalanceLeeAI connects planning, execution, human oversight, evidence, and replay in one auditable workspace. Built in Go, it combines Eino-powered agents, MCP-native tools, RAG knowledge, visual workflows, and attack-chain modeling and analysis for authorized security operations.
...
> [!IMPORTANT]
> Use BalanceLeeAI only on systems you own or are explicitly authorized to test. For shared or production environments, review the [security model](docs/en-US/security-model.md) and [hardening guide](docs/en-US/security-hardening.md) before enabling high-risk tools, WebShell, or C2 capabilities.
...
BalanceLeeAI includes optional integrations under `plugins/`.

- **Burp Suite extension**: `plugins/burp-suite/balanceleeai-burp-extension/`  
  Build output: `plugins/burp-suite/balanceleeai-burp-extension/dist/balanceleeai-burp-extension.jar`  
  Docs: `plugins/burp-suite/balanceleeai-burp-extension/README.md`
- **Browser extension (Chrome / Edge)**: `plugins/browser-extension/balanceleeai-browser-extension/`  
  Capture Network traffic in DevTools and send it to BalanceLeeAI for AI-assisted security testing—aligned with the Burp plugin.  
  Install: `chrome://extensions/` → Load unpacked → F12 → **BalanceLeeAI** tab  
  Package output: `plugins/browser-extension/balanceleeai-browser-extension/dist/balanceleeai-browser-extension.zip`  
  Docs: `plugins/browser-extension/balanceleeai-browser-extension/README.md` / `README.zh-CN.md`

## Tool Overview

BalanceLeeAI ships with 100+ curated tools covering the whole kill chain:
...
```bash
git clone https://github.com/Ed1s0nZ/BalanceLeeAI.git
cd BalanceLeeAI
...
# Manual build
go build -o balancelee-ai cmd/server/main.go
./balancelee-ai --https
```
...
**BalanceLeeAI one-click upgrade:**
...
├── skills/              # Agent Skills dirs (SKILL.md + optional files; demo: balancelee-eino-demo)
...
BalanceLeeAI has joined [404Starlink](https://github.com/knownsec/404StarLink)
...
<img src="./images/wechat-group-balanceleeai-qr.jpg" alt="BalanceLeeAI WeChat group QR code" width="280">
...
BalanceLeeAI is licensed under the Apache License 2.0.  
...
BalanceLeeAI is a professional security testing platform designed to assist security researchers, penetration testers, and IT professionals in conducting security assessments and vulnerability research **with explicit authorization**.
