# BalanceLeeAI Robot / Chatbot Guide
...
## 1. Where to configure in BalanceLeeAI

1. Log in to the BalanceLeeAI web UI.
...
Settings are written to the `robots` section of `config.yaml`; you can also edit the file directly. Web-based **Apply configuration** restarts the corresponding connection automatically. Restart the BalanceLeeAI process only when editing YAML directly. Personal WeChat binding automatically writes `robots.wechat` and restarts the iLink long poll.
...
| WeCom (Qiye WX)| HTTP callback to receive messages; BalanceLeeAI replies via WeCom’s message sending API |
...
Section 3 below describes, per platform, what to do in the developer console and which fields to copy into BalanceLeeAI.
...
If you only have a **custom bot** Webhook URL (`oapi.dingtalk.com/robot/send?access_token=...`) and sign secret (`SEC...`), **do not** put them into BalanceLeeAI. You must create an **enterprise internal app** in the open platform and obtain **Client ID** and **Client Secret** as below.
...
4. **Enable the bot and choose Stream mode**  
   ...
   - **Critical**: set message reception to **“Stream mode”** (流式接入). If you only enable “HTTP callback” or do not select Stream, BalanceLeeAI will not receive messages.  
   - Save.
...
6. **Fill in BalanceLeeAI**  
   - In BalanceLeeAI: System settings → Robot settings → DingTalk.  
   - Enable “Enable DingTalk robot”.  
   - Paste the Client ID and Client Secret from step 3.  
   - Click **Apply configuration**; BalanceLeeAI restarts the DingTalk connection automatically.

---

**Field mapping (DingTalk)**

| Field in BalanceLeeAI | Source in DingTalk Open Platform |
|------------------------|----------------------------------|
...
> - User sends a message → WeCom sends an **encrypted XML callback** to your server (BalanceLeeAI’s `/api/robot/wecom`).  
> - BalanceLeeAI decrypts it, calls the AI, then uses WeCom’s `message/send` API to **actively push the reply** to the user.
...
- In BalanceLeeAI’s `config.yaml`, fill in:
...
> BalanceLeeAI calls `https://qyapi.weixin.qq.com/cgi-bin/message/send` to actively send AI replies.  
> If logs show `errcode 60020 not allow to access from your ip`:
>
> - Your server’s outbound IP is **not in WeCom’s IP allowlist**.  
> - In the WeCom admin console, open the custom app’s **Security / IP allowlist** settings (name may vary slightly), and add the public IP of the machine running BalanceLeeAI (e.g. `110.xxx.xxx.xxx`).  
...
> - Generate a QR code in the BalanceLeeAI web UI → scan and confirm with **WeChat on your phone**;  
...
1. **Log in to BalanceLeeAI web UI**  
...
6. **Test in WeChat**  
   Open the **private chat** with the BalanceLeeAI bot in WeChat and send “帮助” (help) or any text.
...
| Bot Type | Default `3` |
| Bot Agent | Default `BalanceLeeAI/1.0` |
...
| No reply after sleep / network drop | Auto-reconnect in ~5–60 s; restart BalanceLeeAI if still stuck |
...
2. BalanceLeeAI → **System settings** → **Robot settings** → **Telegram**.  
...
4. Paste both tokens in BalanceLeeAI → **Apply configuration**.
...
4. Paste token in BalanceLeeAI; optionally allow guild @ mentions → **Apply configuration**.
...
4. Fill in BalanceLeeAI; use **Sandbox** for testing → **Apply configuration**.

---

## 4. RBAC authorization and bot commands

Platform credentials and callback signatures authenticate the messaging platform. BalanceLeeAI RBAC determines what the sender can actually do. Each bot instance uses one authorization mode.
...
| **版本** (version) | Show current BalanceLeeAI version |
...
1. BalanceLeeAI web UI → System settings → Robot settings → **WeChat / iLink** → **Generate QR code and bind**.  
...
2. **In BalanceLeeAI**: System settings → Robot settings → Enable the platform, paste Client ID/App ID and Client Secret/App Secret → **Apply configuration**.  
...
If the JSON response contains `"reply":"【BalanceLeeAI 机器人命令】..."`, command handling works. `help`, `version`, and `whoami` work before binding. `list`, `current`, and normal AI messages enforce RBAC: the test `platform + user_id` must already be bound or exactly match the service-account allowlist.
...
0. **After laptop sleep or network drop**  
   DingTalk and Lark both use long-lived connections; they break when the machine sleeps or the network drops. The app **auto-reconnects** (retries within about 5–60 seconds). After wake or network recovery, wait a moment before sending; if there is still no response, restart the BalanceLeeAI process.
