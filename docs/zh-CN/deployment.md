...
本文说明 BalanceLeeAI 的常见部署方式。生产环境部署前，请先阅读 [安全模型](security-model.md)，确认授权范围、认证、HITL、审计和高风险功能开关。
...
```bash
go build -o balancelee-ai ./cmd/server
./balancelee-ai --config config.yaml
```
...
server {
    ...
    server_name balancelee.example.com;

    ssl_certificate /etc/letsencrypt/live/balancelee.example.com/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/balancelee.example.com/privkey.pem;
...
```ini
[Unit]
Description=BalanceLeeAI
After=network.target

[Service]
Type=simple
WorkingDirectory=/opt/BalanceLeeAI
ExecStart=/opt/BalanceLeeAI/balancelee-ai --config /opt/BalanceLeeAI/config.yaml
Restart=on-failure
RestartSec=5
...
sudo systemctl enable --now balanceleeai
sudo journalctl -u balanceleeai -f
```
