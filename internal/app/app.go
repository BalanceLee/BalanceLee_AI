...
import (
	...
	"balancelee-ai/internal/agent"
	"balancelee-ai/internal/authctx"
	"balancelee-ai/internal/c2"
	"balancelee-ai/internal/config"
	"balancelee-ai/internal/database"
	"balancelee-ai/internal/einoobserve"
	"balancelee-ai/internal/handler"
	"balancelee-ai/internal/hitl"
	"balancelee-ai/internal/knowledge"
	"balancelee-ai/internal/logger"
	"balancelee-ai/internal/mcp"
	"balancelee-ai/internal/mcp/builtin"
	"balancelee-ai/internal/monitor"
	"balancelee-ai/internal/multiagent"
	"balancelee-ai/internal/security"
	"balancelee-ai/internal/skillpackage"
...
func New(cfg *config.Config, log *logger.Logger, configPath string) (*App, error) {
	...
	// 配置文件路径必须由入口传入（与 flag -config 一致）。勿再用 os.Args[1]，否则 ./balancelee-ai --https 会把 --https 当成路径。
...
func setupRoutes(
	...
	protected.Use(security.AuthMiddleware(authManager))
	{
		...
		protected.POST("/assets/merge", assetHandler.Merge)
