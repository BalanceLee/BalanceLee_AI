...
func (db *DB) BootstrapRBAC(adminPasswordHash string, permissions map[string]string) error {
	// Single-user bootstrap: only the built-in admin credential is required.
	// The legacy role/permission code below is retained solely for source
	// compatibility with old databases and is no longer executed.
	{
		now := time.Now()
		var userCount int
		if err := db.QueryRow(`SELECT COUNT(*) FROM rbac_users`).Scan(&userCount); err != nil {
...
func (db *DB) ResolveRBACAccess(userID string) (*RBACAccess, error) {
	...
	// Single-user mode: the built-in account has global access to every
	// application resource; no role or assignment lookup is needed.
	return &RBACAccess{
		User:        *u,
		Roles:       []RBACRole{{ID: RBACSystemRoleAdmin, Name: RBACSystemRoleAdmin, Scope: RBACScopeAll, IsSystem: true}},
		Permissions: map[string]bool{}, PermissionScopes: map[string]string{}, Scope: RBACScopeAll,
	}, nil
	/* legacy multi-user resolution retained below for old source compatibility */
	rows, err := db.Query(`
