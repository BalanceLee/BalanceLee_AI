...
import (
	...
	"balancelee-ai/internal/einomcp"
	"balancelee-ai/internal/project"
	"balancelee-ai/internal/reasoning"
...
func RunDeepAgent(
	...
	if orchMode != "plan_execute" {
		...
		for _, sub := range effectiveSubs {
			...
			if instr == "" {
				instr = "你是 BalanceLeeAI 中的专业子代理，在授权渗透测试场景下协助完成用户委托的子任务。优先使用可用工具获取证据，回答简洁专业。"
			}
	...
	orchestratorName := "balancelee-deep"
