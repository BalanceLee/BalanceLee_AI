...
func (a *AuthManager) authenticateSession(username, password string) (Session, error) {
	...
	if user.Username != "admin" || !user.IsBuiltin {
		return Session{}, ErrInvalidPassword
	}
	permissions := make(map[string]bool, len(PermissionCatalog))
	permissionScopes := make(map[string]string, len(PermissionCatalog))
	for permission := range PermissionCatalog {
		permissions[permission] = true
		permissionScopes[permission] = database.RBACScopeAll
	}
	return Session{
		Token:            token,
		ExpiresAt:        expiresAt,
