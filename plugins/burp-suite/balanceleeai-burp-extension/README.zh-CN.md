## BalanceLeeAI Burp Suite 插件（中文说明）

### 功能概述

- 在 Burp 的 `BalanceLeeAI` 标签页中配置 **Host、端口、密码**
- 点击 **Validate（验证）**：
  - 调用 `POST /api/auth/login` 用密码换取 Token
  - 调用 `GET /api/auth/validate` 校验 Token
  - 验证通过后 Token 会保存在插件内存中（本次 Burp 会话有效）
- 右键任意 HTTP 请求包 → **Send to BalanceLeeAI (stream test)**：
  - 弹出发送对话框，可针对**当前流量**选择：
...
```bash
cd plugins/burp-suite/balanceleeai-burp-extension
./build-mvn.sh
```

产物：

- `dist/balanceleeai-burp-extension.jar`
...
- Extension type：**Java**
- 选择 `dist/balanceleeai-burp-extension.jar`

### 使用方法

1) 打开 Burp 顶部标签页 `BalanceLeeAI`
2) 填写：
   - **Host**：例如 `127.0.0.1`
   - **Port**：例如 `8080`
   - **HTTPS**：默认勾选（对接 `config.yaml` 中 `tls_enabled` / 自签证书）；插件会自动信任本地自签证书，无需导入
   - **Password**：你的 BalanceLeeAI 登录密码（对应服务端 `auth.password`）
3) 点击 **Validate**
   - 成功：状态显示 `OK (token saved)`
   - 失败：状态会显示错误原因（例如密码错误、服务不可达、401/403 等）
4) 在 Burp 的 Proxy/HTTP history/Repeater 等列表中选中一条 HTTP 包
5) 右键 → **Send to BalanceLeeAI (stream test)**
6) 在弹出框中按需选择**项目、角色、对话模式**，并编辑测试指令后点确定
7) 每次发送后会在 `BalanceLeeAI` 标签页左侧显示一个“测试记录”（请求标题 + 模式/角色 + 状态）；点击对应记录即可在右侧查看该次的流式输出结果
...
- **右键发送后无流式输出**
  ...
  - 确认 Burp 能访问到 BalanceLeeAI（网络/代理/防火墙）
