...
final class BalanceLeeAITab implements ITab {
    ...
    private JComponent buildConfigPanel() {
        ...
        row1.add(portField);
        useHttpsBox.setToolTipText("Use https:// for BalanceLeeAI (self-signed certs are trusted automatically)");
    ...
    private void wireActions() {
        validateButton.addActionListener(e -> {
            ...
            Thread worker = new Thread(() -> {
                try {
                    BalanceLeeAIClient.Config cfg = currentConfig();
    ...
    BalanceLeeAIClient.Config currentConfig() {
        String host = hostField.getText().trim();
        String port = portField.getText().trim();
        String password = new String(passwordField.getPassword());
        String scheme = useHttpsBox.isSelected() ? "https" : "http";
        String baseUrl = scheme + "://" + host + ":" + port;
        return new BalanceLeeAIClient.Config(baseUrl, password);
    }
