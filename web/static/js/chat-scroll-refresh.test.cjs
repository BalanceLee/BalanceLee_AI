...
function createScrollRuntime() {
    ...
    return {
        api: window.BalanceLeeChatScroll,
        chatEl,
        returnLatest,
        listeners,
...
test('页面在任务补流脚本之前加载智能滚动控制器', () => {
    ...
    const monitorIndex = html.indexOf('/static/js/monitor.js?v=20260913-beliefpath-summary-1');
...
test('刷新指定对话时立即恢复且加载完成前不闪出无项目状态', () => {
    ...
    assert.match(html, /chat\.js\?v=20260913-beliefpath-summary-1/);
});
...
test('暗色模式对话三点悬浮不会触发浅色父行背景', () => {
    ...
    assert.match(html, /style\.css\?v=20260913-beliefpath-summary-1/);
});
