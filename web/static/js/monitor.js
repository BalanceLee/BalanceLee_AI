...
const CHAT_TASK_SYNC_CHANNEL_NAME = 'balancelee-chat-task-sync-v1';
...
function startProcessDetailsLatestFollow(assistantMessageId, options) {
    ...
    const followLatest = function () {
        ...
        if (window.BalanceLeeChatScroll &&
            typeof window.BalanceLeeChatScroll.scrollIfPinned === 'function') {
            window.BalanceLeeChatScroll.scrollIfPinned(true);
        }
    };
...
function toggleProcessDetails(progressId, assistantMessageId) {
    ...
    if (timeline && timeline.classList.contains('expanded')) {
        setTimeout(() => {
            if (window.BalanceLeeChatScroll && typeof window.BalanceLeeChatScroll.scrollIntoViewIfFollowing === 'function') {
                window.BalanceLeeChatScroll.scrollIntoViewIfFollowing(detailsContainer, { behavior: 'smooth', block: 'nearest' });
...
function describeHitlApprovalRequest(data) {
    ...
    let question = hitlApprovalTemplate('hitl.requestGeneric', '允许 BalanceLeeAI 调用 {{tool}}？', { tool: rawToolName });
    let primary = '';
    let kind = 'generic';
    if (isBrowser) {
        kind = 'browser';
        question = url
            ? (url.length > 160
                ? hitlApprovalTranslate('hitl.requestVisitLongUrl', '允许 BalanceLeeAI 访问此地址？')
                : hitlApprovalTemplate('hitl.requestVisitUrl', '允许 BalanceLeeAI 访问 {{url}}？', { url: url }))
            : hitlApprovalTranslate('hitl.requestBrowser', '允许 BalanceLeeAI 使用浏览器？');
        primary = url;
    } else if (isCommand) {
        kind = 'command';
        question = hitlApprovalTranslate('hitl.requestCommand', '允许 BalanceLeeAI 执行这条命令？');
        primary = command;
    } else if (isFile) {
        kind = 'file';
        question = path
            ? (path.length > 160
                ? hitlApprovalTranslate('hitl.requestModifyLongPath', '允许 BalanceLeeAI 修改此文件？')
                : hitlApprovalTemplate('hitl.requestFile', '允许 BalanceLeeAI 修改 {{path}}？', { path: path }))
            : hitlApprovalTranslate('hitl.requestFiles', '允许 BalanceLeeAI 修改文件？');
        primary = path;
    }
    return {
...
async function reconcileConversationAfterTaskReplay(conversationId, keepFollowing) {
    ...
    if (keepFollowing && window.BalanceLeeChatScroll) {
        if (typeof window.BalanceLeeChatScroll.settleToBottomIfFollowing === 'function') {
            window.BalanceLeeChatScroll.settleToBottomIfFollowing(24);
        } else if (typeof window.BalanceLeeChatScroll.forceScrollToBottom === 'function') {
            window.BalanceLeeChatScroll.forceScrollToBottom(false);
        }
    }
...
async function attachRunningTaskEventStream(conversationId) {
    ...
    const attachPromise = (async function () {
        try {
            ...
            if (window.BalanceLeeChatScroll && typeof window.BalanceLeeChatScroll.onTaskEventStreamBegin === 'function') {
                window.BalanceLeeChatScroll.onTaskEventStreamBegin(conversationId, asEl.id, progressId);
            }
            ...
            if (
                window.BalanceLeeChatScroll &&
                typeof window.BalanceLeeChatScroll.settleToBottomIfFollowing === 'function' &&
                (typeof window.captureScrollPinState !== 'function' || window.captureScrollPinState())
            ) {
                window.BalanceLeeChatScroll.settleToBottomIfFollowing(12);
            } else if (
                window.BalanceLeeChatScroll &&
                typeof window.BalanceLeeChatScroll.forceScrollToBottom === 'function' &&
                (typeof window.captureScrollPinState !== 'function' || window.captureScrollPinState())
            ) {
                window.BalanceLeeChatScroll.forceScrollToBottom(false);
            }
            ...
            if (!response.ok) {
                ...
                if (window.BalanceLeeChatScroll && typeof window.BalanceLeeChatScroll.onTaskEventStreamEnd === 'function') {
                    window.BalanceLeeChatScroll.onTaskEventStreamEnd();
                }
                await reconcileConversationAfterTaskReplay(conversationId, true);
                return false;
            }
            ...
            if (replaySawDone && typeof window.loadConversation === 'function' && window.currentConversationId === conversationId) {
                ...
                if (
                    ...
                    window.BalanceLeeChatScroll.settleToBottomIfFollowing(18);
                } else if (
                    keepFollowingFinalRender &&
                    window.BalanceLeeChatScroll &&
                    typeof window.BalanceLeeChatScroll.forceScrollToBottom === 'function'
            ...
            if (ownsCurrentAttach && window.BalanceLeeChatScroll && typeof window.BalanceLeeChatScroll.onTaskEventStreamEnd === 'function') {
                window.BalanceLeeChatScroll.onTaskEventStreamEnd();
            }
...
function isToolGuardBlockedResult(value, depth, allowLegacy) {
    ...
    if (value._meta && value._meta['balancelee.ai/blocked'] === true) return true;
...
if (typeof document !== 'undefined' && !document.__balanceLeeToolCallDetailToggleBound) {
    document.__balanceLeeToolCallDetailToggleBound = true;
